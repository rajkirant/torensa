...
ENV_VOICE_CHANGE_R2_OBJECT_PREFIX = "VOICE_CHANGE_R2_OBJECT_PREFIX"
ENV_VOICE_CHANGE_R2_UPLOAD_URL_TTL = "VOICE_CHANGE_R2_UPLOAD_URL_TTL"
ENV_VOICE_CHANGE_R2_DOWNLOAD_URL_TTL = "VOICE_CHANGE_R2_DOWNLOAD_URL_TTL"
...
def _get_voice_r2_prefix() -> str:
    prefix = os.getenv(ENV_VOICE_CHANGE_R2_OBJECT_PREFIX, "voice-change").strip().strip("/")
    return prefix or "voice-change"
...
def voice_upload_init_view(request):
    ...
    prefix = _get_voice_r2_prefix()
    object_key = f"{prefix}/input/{uuid.uuid4().hex}-{Path(filename).name}"
    ttl = _env_int(ENV_VOICE_CHANGE_R2_UPLOAD_URL_TTL, DEFAULT_UPLOAD_URL_TTL)
...
@api_view(["POST"])
@permission_classes([AllowAny])
def voice_change_from_r2_view(request):
    if not is_r2_enabled():
        return Response(
            {"error": "Cloudflare R2 is not enabled on this server."},
            status=503,
    ...
    output_name = f"voice-{preset}.{output_format}"
    content_type = "audio/wav" if output_format == "wav" else "audio/mpeg"
    ttl = _env_int(ENV_VOICE_CHANGE_R2_DOWNLOAD_URL_TTL, DEFAULT_DOWNLOAD_URL_TTL)
