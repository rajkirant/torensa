...
ENV_NOISE_REMOVE_MAX_UPLOAD_MB = "NOISE_REMOVE_MAX_UPLOAD_MB"
ENV_NOISE_REMOVE_R2_OBJECT_PREFIX = "NOISE_REMOVE_R2_OBJECT_PREFIX"
ENV_NOISE_REMOVE_R2_UPLOAD_URL_TTL = "NOISE_REMOVE_R2_UPLOAD_URL_TTL"
ENV_NOISE_REMOVE_R2_DOWNLOAD_URL_TTL = "NOISE_REMOVE_R2_DOWNLOAD_URL_TTL"
...
# Uses FFmpeg's afftdn (FFT-based denoiser) + highpass to cut rumble
STRENGTH_FILTERS: dict[str, str] = {
    "light": "highpass=f=80,afftdn=nf=-20",
    "medium": "highpass=f=100,afftdn=nf=-30,lowpass=f=15000",
    "strong": "highpass=f=120,afftdn=nf=-40,lowpass=f=13000",
}
...
def _normalize_strength(value: str | None) -> str:
    if not value:
        return "medium"
    normalized = value.strip().lower()
    return normalized if normalized in STRENGTH_FILTERS else "medium"


def _get_noise_r2_prefix() -> str:
    prefix = os.getenv(ENV_NOISE_REMOVE_R2_OBJECT_PREFIX, "noise-remove").strip().strip("/")
    return prefix or "noise-remove"
...
def _run_ffmpeg(
    ...
    filter_chain = STRENGTH_FILTERS[strength]
...
def noise_upload_init_view(request):
    ...
    max_upload_mb = _env_int(ENV_NOISE_REMOVE_MAX_UPLOAD_MB, DEFAULT_MAX_UPLOAD_MB)
    ...
    prefix = _get_noise_r2_prefix()
    object_key = f"{prefix}/input/{uuid.uuid4().hex}-{Path(filename).name}"
    ttl = _env_int(ENV_NOISE_REMOVE_R2_UPLOAD_URL_TTL, DEFAULT_UPLOAD_URL_TTL)
...
def noise_remove_view(request):
    ...
    strength = _normalize_strength(request.data.get("strength"))
    ffmpeg = _find_ffmpeg()
    if ffmpeg is None:
        return Response(
            {
    ...
    with tempfile.TemporaryDirectory(prefix="torensa_noise_") as tmpdir:
        ...
        try:
            return_code, stderr = _run_ffmpeg(
                ffmpeg=ffmpeg,
            ...
            return Response(
                {"error": "Noise removal timed out. Please try a smaller file."},
                status=500,
            )
        ...
        if return_code != 0:
            ...
            return Response(
                {"error": "Noise removal failed. Please try another file."},
                status=500,
            )

        if not Path(output_path).exists():
            return Response(
                {"error": "Processing failed to produce output audio."},
                status=500,
            )
    ...
    output_name = f"{original_stem}-clean.{output_format}"
...
@api_view(["POST"])
@permission_classes([AllowAny])
def noise_remove_from_r2_view(request):
    if not is_r2_enabled():
        return Response(
            {"error": "Cloudflare R2 is not enabled on this server."},
            status=503,
    ...
    output_name = f"audio-clean.{output_format}"
    content_type = "audio/wav" if output_format == "wav" else "audio/mpeg"
    ttl = _env_int(ENV_NOISE_REMOVE_R2_DOWNLOAD_URL_TTL, DEFAULT_DOWNLOAD_URL_TTL)
