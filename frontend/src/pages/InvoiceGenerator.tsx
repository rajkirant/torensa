...
async function generatePdf(form: FormState, items: LineItem[]) {
    ...
    drawText(form.docType === "invoice" ? "Invoice #" : "Receipt #", metaX, metaY, 10, true, gray);
...
export default function InvoiceGenerator() {
    ...
    const download = async () => {
        try {
            ...
            if (!form.sellerName.trim()) return setError("Please add your seller/business name.");
            if (!form.billToName.trim()) return setError("Please add a Bill To name.");
            const validItems = items.filter((it) => it.description.trim() && it.quantity > 0);
            if (validItems.length === 0) return setError("Please add at least one line item.");
            ...
            setError(e?.message ? `Failed to generate PDF: ${e.message}` : "Failed to generate PDF.");
        } finally {
            setBusy(false);
        }
    };
    ...
    return (
        <Card sx={{ maxWidth: 820, mx: "auto", mt: 6 }}>
            <CardContent>
                <Stack spacing={3}>
                    <Stack direction="row" alignItems="center" justifyContent="space-between" gap={2}>
                        ...
                        <Chip label="Offline-ready" variant="outlined" />
                    </Stack>

                    <Typography variant="body2" color="text.secondary">
                        This tool works offline after you&apos;ve opened it once while connected.
                        Just bookmark <strong>torensa.com/invoice-generator</strong> in your browser
                        and you can create invoices/receipts even without an internet connection.
                        Using it offline also means fewer repeated network calls over time, which can
                        save a bit of data and reduce server load.
                    </Typography>
                        ...
                        {items.map((it) => (
                            ...
                            >
                                ...
                                <TextField
                                    ...
                                    value={it.quantity}
                                    onChange={(e) =>
                                        updateItem(it.id, { quantity: safeNumber(e.target.value, 1) })
                                    }
                                ...
                                <Stack direction="row" spacing={1} alignItems="center">
                                    ...
                                    <IconButton aria-label="Remove item" onClick={() => removeItem(it.id)}>
                                        <DeleteIcon />
                                    </IconButton>
                                </Stack>
                            </Stack>
                        ...
                        <Stack spacing={2} flex={1}>
                            <Stack direction="row" spacing={2}>
                                <TextField
                                    ...
                                    type="number"
                                    value={form.taxRate}
                                    onChange={(e) => updateForm("taxRate", safeNumber(e.target.value, 0))}
                                    inputProps={{ min: 0, step: "0.01" }}
                                    fullWidth
                                />
                                <TextField
                                    ...
                                    type="number"
                                    value={form.shipping}
                                    onChange={(e) => updateForm("shipping", safeNumber(e.target.value, 0))}
                                    inputProps={{ min: 0, step: "0.01" }}
                                    fullWidth
                                />
                                <TextField
                                    ...
                                    type="number"
                                    value={form.discount}
                                    onChange={(e) => updateForm("discount", safeNumber(e.target.value, 0))}
                                    inputProps={{ min: 0, step: "0.01" }}
                                    fullWidth
                                />
                            </Stack>
                            ...
                            >
                                <Stack direction="row" justifyContent="space-between">
                                    ...
                                    <Typography variant="body2">{money(subtotal, form.currency)}</Typography>
                                </Stack>
                                ...
                                <Stack direction="row" justifyContent="space-between">
                                    ...
                                    <Typography variant="body2">{money(taxAmount, form.currency)}</Typography>
                                </Stack>
