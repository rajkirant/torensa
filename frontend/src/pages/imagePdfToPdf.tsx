...
const ImagePdfToPdf: React.FC = () => {
            ...
            if (kind === "image") {
                ...
                (async () => {
                    try {
                        ...
                        setItems((prev) =>
                            prev.map((p) => (p.id === item.id ? { ...p, pdfPageCount: pdf.getPageCount() } : p)),
                        );
                    } catch {
        ...
        setItems((prev) =>
            prev.map((p) =>
                p.id === id
                    ? {
                        ...p,
                        imageRotation: ((p.imageRotation + 90) % 360) as 0 | 90 | 180 | 270,
                    }
                    : p,
            ),
        );
    ...
    const canBuild = useMemo(() => items.length > 0 && !busy, [items.length, busy]);

    const offlineHint = useMemo(() => {
        // Keep it similar to your QR copy
        return (
            <>
                This tool runs <strong>fully in your browser</strong>. Once you’ve opened it at least once while
                connected, you can use it offline (great for privacy + fewer network calls). Bookmark{" "}
    ...
    const buildPdf = async () => {
        ...
        try {
            ...
            // For page numbers
            const font = pageNumbers ? await out.embedFont(StandardFonts.Helvetica) : null;
            ...
            setError(e?.message || "Failed to build PDF. Please try again with different files.");
        } finally {
            setBusy(false);
        }
    };
    ...
    return (
        <Card sx={{ maxWidth: 920, mx: "auto", mt: 6 }}>
            <CardContent>
                <Stack spacing={2.5}>
                    <Stack spacing={0.5}>
                                                ...
                                                <Stack spacing={0.25} sx={{ minWidth: 0, flex: 1 }}>
                                                    <Stack direction="row" spacing={1} alignItems="center">
                                                        ...
                                                        <Chip
                                                            ...
                                                            variant="outlined"
                                                            icon={it.kind === "pdf" ? <PictureAsPdfIcon /> : <ImageIcon />}
                                                        />
                                                    </Stack>
                    ...
                    <Stack spacing={2}>
                        ...
                        <Alert severity="info">
                            Tip: For mixed inputs, you can reorder the queue. Images become pages; PDFs are appended page-by-page.
